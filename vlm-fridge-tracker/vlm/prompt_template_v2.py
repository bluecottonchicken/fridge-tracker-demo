"""Prompt 模板 v2：精简去重版，与 v1 保持相同接口"""
...
3. 首尾帧对比仅用于辅助验证手部动作，不能单独作为事件依据。
4. 物品名称用日常用语（如"牛奶"而非"乳制品饮料"）。无法确定时，必须详细描述容器特征（材质、透明度、颜色、形状、标签文字），禁止输出"食物容器"等笼统描述。
5. 宁可漏报也不要误报。不确定时标注 confidence < 0.5。"""
...
请逐帧追踪人手位置和手中物品，判断手的运动方向（伸入冰箱=put_in，移出冰箱=take_out），输出以下 JSON：
{{
  "hand_observations": [
    {{
      "frame_number": 帧编号,
