...
def get_last_fridge_state(db: Session, user_id: int) -> dict[str, int]:
    """基于所有历史事件 SQL 聚合计算当前冰箱库存，返回 {物品名: 数量}"""
    qty_expr = func.sum(
        case(
    ...
    stmt = (
        select(InventoryEvent.item, qty_expr)
        .where(InventoryEvent.user_id == user_id)
        .group_by(InventoryEvent.item)
        .having(qty_expr > 0)
    )

    return {row.item: int(row.net_qty) for row in db.exec(stmt).all()}
